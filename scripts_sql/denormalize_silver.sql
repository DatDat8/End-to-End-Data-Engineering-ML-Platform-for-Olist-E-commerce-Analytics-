CREATE OR REPLACE TABLE silver.denormalized_orders AS
  ...
  SELECT
    ...
    ARRAY_AGG(
      STRUCT(
        ...
        t1.price,
        t1.freight_value,
        INITCAP(REPLACE(CASE t3.product_category_name_english
          WHEN NULL THEN t2.product_category_name
          ELSE t3.product_category_name_english
...
-- Aggreated over order_id to merge with orders
payments_agg AS (
  SELECT
    order_id,
...
sellers_agg AS (
  SELECT
    t1.order_id,
    -- FIRST_VALUE(t2.seller_state) OVER(PARTITION BY t1.order_id ORDER BY t2.seller_id) , 
    ARRAY_AGG(
      STRUCT(
        t2.seller_id,
  ...
  FROM `bronze.order_items` AS t1
  LEFT JOIN `bronze.sellers` AS t2
...
core_order AS (
  ...
  LEFT JOIN `bronze.customers` AS t2
