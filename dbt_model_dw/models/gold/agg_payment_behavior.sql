{{
  config(
    materialized='table',
    schema='gold',
    alias='agg_payment_behavior',
    tags=['aggregate', 'payment', 'risk']
  )
}}
...
SELECT
    ...
    COUNT(t1.order_id) AS total_order_items, -- Renamed for clarity since UNNEST creates one row per item
    ROUND(SUM(t1.calculated_order_cost),2) AS total_gmv,
    ROUND(SUM(t1.calculated_order_cost - t1.total_payment_value),2) AS payment_missing_value, -- Renamed for clarity
    ...
    ROUND(SUM(t1.review_score),2) AS total_review_score,
